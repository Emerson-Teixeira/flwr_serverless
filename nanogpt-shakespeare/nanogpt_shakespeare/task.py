...
def load_data(partition_id: int, num_partitions: int, batch_size: int, block_size: int):
    """Load a partition of Shakespeare data and return train/val dataloaders."""
    text = _download_shakespeare()
    meta = _get_meta()
    ids = np.array(_encode(text, meta["stoi"]), dtype=np.uint16)

    # Split into train/val first (90/10), then partition the train set
    n = len(ids)
    split = int(n * 0.9)
    train_ids = ids[:split]
    val_ids = ids[split:]

    # Partition the training data among clients (contiguous chunks)
    ...
    # Each client uses the full val set for evaluation
...
# Defaults matching pyproject.toml (needed because Ray client workers
# may not receive run_config in simulation mode).
DEFAULTS = {
    "block-size": 256,
    "n-layer": 6,
    "n-head": 6,
...
def _cfg(run_config: dict, key: str):
    return run_config.get(key, DEFAULTS[key])
...
def build_model(run_config: dict) -> GPT:
    ...
    meta = _get_meta()
    config = GPTConfig(
        block_size=int(_cfg(run_config, "block-size")),
        vocab_size=meta["vocab_size"],
        n_layer=int(_cfg(run_config, "n-layer")),
        n_head=int(_cfg(run_config, "n-head")),
        n_embd=int(_cfg(run_config, "n-embd")),
        dropout=float(_cfg(run_config, "dropout")),
    )
    return GPT(config)
